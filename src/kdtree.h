...
struct KdTree
{
	...
	void searchHelper(Node* node, std::vector<float> target, std::vector<int>& ids, int depth, float distanceTol){
		...
		if(node != NULL){
			...
			float distances[] = {target[0]-node->point[0],target[1]-node->point[1],target[2]-node->point[2] };
			float distance = sqrt(distances[0]*distances[0] + distances[1]*distances[1] + distances[2]*distances[2]);
